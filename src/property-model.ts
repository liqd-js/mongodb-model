...
export abstract class AbstractPropertyModel<
    ...
    SmartFilters extends AbstractSmartFilters<SmartFilters>,
...
>
{
    ...
    protected async pipeline( list: PropertyModelListOptions<RootDBE, DBE, SmartFilters> = {} ): Promise<Document[]>
    {
    ...
    public async list<K extends keyof Extensions['converters']>(list: PropertyModelListOptions<RootDBE, DBE, SmartFilters>, conversion: K = 'dto' as K ): Promise<WithTotal<Array<Awaited<ReturnType<Extensions['converters'][K]['converter']>>>>>
    {
        const { converter, projection } = this.converters[conversion];
        const prev = list.cursor?.startsWith('prev:');
    ...
    public async aggregate<T>( pipeline: Document[], options?: PropertyModelAggregateOptions<RootDBE, DBE, SmartFilters> ): Promise<T[]>
    {
    ...
    public async count( pipeline: Document[], options?: PropertyModelAggregateOptions<RootDBE, DBE, SmartFilters> ): Promise<number>
    {
    ...
    public async resolveSmartFilter( smartFilter: {[key in PublicMethodNames<SmartFilters>]: any} ): Promise<{ filter?: Filter<DBE>, pipeline?: Document[] }>
    {
        const pipeline: any[] = [];
        let filter: any = {};
    ...
    private async filterStages( list: PropertyModelListOptions<RootDBE, DBE, SmartFilters> )
    {
    ...
    private splitProjection( projection: PropertyModelListOptions<RootDBE, DBE, SmartFilters>['projection'] ): {rootProjection?: any, propertyProjection?: any}
    {
